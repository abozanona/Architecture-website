...
if ( !class_exists( 'UFBL_Class' ) ) {
	...
	class UFBL_Class {
		 ...
		 */
		function ufbl_menu() {
			add_menu_page( 'نماذج الإدخال', 'نماذج الإدخال', 'manage_options', UFBL_SLUG, array( $this, 'forms_list' ),'dashicons-welcome-widgets-menus',35.7 );
			add_submenu_page( UFBL_SLUG, isset( $_GET['form_id'] ) ? 'تعديل النموذج' : 'جميع النماذج', 'جميع النماذج', 'manage_options', UFBL_SLUG, array( $this, 'forms_list' ) );
			add_submenu_page( UFBL_SLUG, 'نموذج جديد', 'نموذج جديد', 'manage_options', 'ufbl-new-form', array( $this, 'add_new_form' ) ); 
			add_submenu_page( UFBL_SLUG, 'إدخالات المستخدمين', 'إدخالات المستخدمين', 'manage_options', 'ufbl-form-entries', array( $this, 'forms_entries' ) );
		...
		function register_frontend_assets() {
			...
			$frontend_js_obj = array(
				'default_error_message' => 'هذا الحقل إجباري',
