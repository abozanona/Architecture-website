...
<div class="ufbl-form-wrapper <?php echo $form_template_class; ?>" style="width:<?php echo $form_width; ?>;">
    <form method="post" action="" class="ufbl-front-form">
		<?php if ( !isset( $form_design['hide_form_title'] ) ) { ?><div class="ufbl-form-title"><?php echo (isset( $form_row['form_title'] ) && $form_row['form_title'] != '') ? esc_attr( $form_row['form_title'] ) : 'نموذج التواصل'; ?></div><?php } ?>
		...
		foreach ( $form_detail['field_data'] as $key => $val ) {
			if ( isset( $val['field_type'] ) ) {
				...
				switch ( $val['field_type'] ) {
					...
					case 'submit':
						?>
						<div class="text-center ufbl-form-field-wrap <?php echo ($val['field_class'] != '') ? esc_attr( $val['field_class'] ) : ''; ?>" <?php echo ($val['field_id'] != '') ? 'id="' . esc_attr( $val['field_id'] ) . '"' : ''; ?>>
							<div class="ufbl-form-field">
								<input type="submit" class="btn btn-primary newbtnclass" name="<?php echo $key; ?>" value="<?php echo (isset( $val['button_label'] ) && $val['button_label'] != '') ? esc_attr( $val['button_label'] ) : 'إرسال المعلومات'; ?>"/>
								<?php if(isset($val['show_reset_button'])){?>
								<input type="reset" class="ufbl-form-reset" value="<?php echo (isset( $val['reset_label'] ) && $val['reset_label'] != '') ? esc_attr( $val['reset_label'] ) : 'إفراغ الحقول'; ?>"/>
								<?php }?>
							...
							<div class="ufbl-form-field">
								...
								if ( $val['captcha_type'] == 'mathematical' ) {
									...
									<div class="ufbl-math-captcha-wrap">
										<span class="ufbl-captcha-num"><?php echo $num1; ?></span> + <span><?php echo $num2; ?></span> = <input type="text" name="<?php echo $key; ?>" class="ufbl-math-captcha-ans" placeholder="<?php echo (isset( $val['placeholder'] ) && $val['placeholder'] != '') ? esc_attr( $val['placeholder'] ) : 'أدخل مجموع الرقمين'; ?>"/>
