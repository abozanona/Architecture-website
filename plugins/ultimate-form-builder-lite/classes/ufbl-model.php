...
if ( !class_exists( 'UFBL_Model' ) ) {
	...
	class UFBL_Model {
		...
		public static function add_form() {
			...
			$form_title = ($form_title == '') ? 'نموذج الإدخال' : $form_title;
			$form_status = 1;
			...
			if ( $check == 1 ) {
				...
				$response_array = array( 'success' => 0, 'error_msg' => 'حدث عطل غير متوقع, يرجى المحاولة فيما بعد' );
			}
		...
		public static function change_form_status() {
			...
			$msg = ($check) ? 'تم تحديث الحالة' : 'حدث عطل أثناء التنفيذ';
			die( $msg );
		}
			...
			if ( !empty( $form_row ) ) {
				...
				foreach ( $form_detail['field_data'] as $field_key => $field_settings ) {
					if ( !in_array( $field_settings['field_type'], $except_field_types ) ) {
						$form_labels[] = (isset( $field_settings['field_label'] ) && $field_settings['field_label'] != '') ? esc_attr( $field_settings['field_label'] ) : 'بدون عنوان' . ' - ' . $field_settings['field_type'];
						$form_keys[] = $field_key;
					}
				}
		...
		public static function copy_form() {
			...
			if ( $check == 1 ) {
				...
				$response_array = array( 'success' => 0, 'error_msg' => 'حدث خطأ غير متوقع, يرجى المحاولة فيما بعد.' );
			}
			die( json_encode( $response_array ) );
		}
